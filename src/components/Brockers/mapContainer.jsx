...
const MapContainer = forwardRef(({ lat: externalLat, lon: externalLng, direccion: externalDireccion, onMarkerDragEnd: externalOnMarkerDragEnd, onUpdateLocation }, ref) => {
  ...
  const handleAddressChange = (event) => {
    ...
    setAddress(newAddress);
    externalOnMarkerDragEnd({ lat: position.lat, lng: position.lng, direccion: newAddress });
  };
  ...
  const handleSearchLocation = () => {
    getLatLngFromAddress(address)
      ...
      .catch((error) => console.error('Error al obtener la latitud y longitud:', error));
  };
  ...
  const getLatLngFromAddress = async (address) => {
    try {
      ...
      throw new Error('Error al obtener la latitud y longitud desde la dirección.');
    }
  };
  ...
  useEffect(() => {
    setPosition({ lat: externalLat || -2.1894128, lng: externalLng || -79.8890662 });
  ...
  const getAddressFromLatLng = async (lat, lng) => {
    try {
      ...
      throw new Error('Error al obtener la dirección desde la latitud y longitud.');
    }
  };
  ...
  return (
    <LoadScript googleMapsApiKey="AIzaSyCzH9Z4WlK74-EgYlMR7BLB9SUnkTfouIE" >
      <div>
        ...
        <button style={{display:'none'}} onClick={handleSearchLocation}>Buscar Ubicación</button>
      </div>
      <div>
        <label style={{display:'none'}}>
          Latitud:
      ...
      <GoogleMap
        ...
        onClick={onMapClick}
      >
        {marker && <Marker position={marker} draggable onDragEnd={onMarkerDragEnd} />}
      </GoogleMap>
    </LoadScript>
  );
});
